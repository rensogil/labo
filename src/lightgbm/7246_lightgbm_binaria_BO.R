...
dataset[ , campo7 := as.integer( ctrx_quarter>=20 & ctrx_quarter>=41  & rank_cpayroll<0.42177  ) ]
dataset[ , campo8 := as.integer( ctrx_quarter>=20 & ctrx_quarter>=41  & rank_cpayroll>=0.42177 ) ]
...
for (campo in variables_rank) {
  ...
  dataset[    , paste0("rank_", campo) := (frankv(dataset, cols = campo) - 1) / (length(dataset[, get(campo)]) - 1)] # rankeo entre 0 y 1
  dataset[, paste0(campo) := NULL] 
} 
